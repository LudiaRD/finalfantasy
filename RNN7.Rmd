...
```{r}
X <- as.matrix(cbind(train_norm[1:906, 1], train_norm[2:907, 1], train_norm[3:908, 1], train_norm[4:909, 1], train_norm[5:910, 1], train_norm[6:911, 1], train_norm[7:912, 1]))
Y <- as.matrix(train_norm[8:913, 1])

X1 <- as.matrix(cbind(test_norm[1:385, 1], test_norm[2:386, 1], test_norm[3:387, 1], test_norm[4:388, 1], test_norm[5:389, 1], test_norm[6:390, 1], test_norm[7:391, 1]))
Y1 <- as.matrix(test_norm[8:392, 1])
```
#MODEL_UNIVARIATE
```{r}
#looping model
    ...
    for (k in 7:21) {
      ...
      #running time
        ...
        #export
          ...
          write.table(mape_uni, "mape-uni-7.csv", row.names = F, col.names = F, append = T, sep = ",")
        }
      }
    }
  }
...
```{r}
X2 <- as.matrix(cbind(train_norm[1:906, c(1,2)], train_norm[2:907, c(1,2)], train_norm[3:908, c(1,2)], train_norm[4:909, c(1,2)], train_norm[5:910, c(1,2)], train_norm[6:911, c(1,2)], train_norm[7:912, c(1,2)]))
Y2 <- as.matrix(train_norm[8:913, 1])

X3 <- as.matrix(cbind(test_norm[1:385, c(1,2)], test_norm[2:386, c(1,2)], test_norm[3:387, c(1,2)], test_norm[4:388, c(1,2)], test_norm[5:389, c(1,2)], test_norm[6:390, c(1,2)], test_norm[7:391, c(1,2)]))
Y3 <- as.matrix(test_norm[8:392, 1])
```
#MODEL_MULTIVARIATE
```{r}
#looping model
    ...
    for (k in 14:42) {
      ...
      #running time
        ...
        #export
          ...
          mape_uni <- cbind(i, j, k, l, m, eror, time_taken)
          write.csv(predict_uni, file = form, row.names = F)
          write.table(mape_uni, "mape-multi-7.csv", row.names = F, col.names = F, append = T, sep = ",")
        }
      }
    }
  }
