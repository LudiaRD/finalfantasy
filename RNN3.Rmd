...
```{r}
X <- as.matrix(cbind(train_norm[1:910, 1], train_norm[2:911, 1], train_norm[3:912, 1]))
Y <- as.matrix(train_norm[4:913, 1])

X1 <- as.matrix(cbind(test_norm[1:389, 1], test_norm[2:390, 1], test_norm[3:391, 1]))
Y1 <- as.matrix(test_norm[4:392, 1])
```
#MODEL_UNIVARIATE
```{r}
#looping model
    ...
    for (k in 3:9) {
      ...
      #running time
        ...
        #export
          ...
          write.table(mape_uni, "mape-uni-3.csv", row.names = F, col.names = F, append = T, sep = ",")
        }
      }
    }
  }
...
```{r}
X2 <- as.matrix(cbind(train_norm[1:910, c(1,2)], train_norm[2:911, c(1,2)], train_norm[3:912, c(1,2)]))
Y2 <- as.matrix(train_norm[4:913, 1])

X3 <- as.matrix(cbind(test_norm[1:389, c(1,2)], test_norm[2:390, c(1,2)], test_norm[3:391, c(1,2)]))
Y3 <- as.matrix(test_norm[4:392, 1])
```
#MODEL_MULTIVARIATE
```{r}
#looping model
    ...
    for (k in 6:18) {
      ...
      #running time
        ...
        #export
          ...
          mape_uni <- cbind(i, j, k, l, m, eror, time_taken)
          write.csv(predict_uni, file = form, row.names = F)
          write.table(mape_uni, "mape-multi-3.csv", row.names = F, col.names = F, append = T, sep = ",")
        }
      }
    }
  }
